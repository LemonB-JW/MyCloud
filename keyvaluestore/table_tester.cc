...
int main(int argc, char const *argv[])
{
	...
	bool ret = table.put("2019/11/20", 0, "foo.txt", "file", "alice", id1, data);
	string val = table.get("alice", "foo.txt");
	cout<<"get val "<<val<<endl;
	bool ret2 = table.cput("2019/11/20", 97, "foo.txt", "file", "alice", id1, data, new_data);
	...
	string id2 = "boo.txt";
	bool ret3 = table.put("2019/11/20", 10, "boo.txt", "file", "alice", id2, data2);
	vector<string> all_files = table.list_all_files_for_currUser("alice");
	for(int i = 0; i < all_files.size(); i++){
		cout<<"file info is "<<all_files.at(i)<<endl;
	}
	return 0;
}