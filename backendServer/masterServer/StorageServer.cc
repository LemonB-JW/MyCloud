...
int main(int argc, char *argv[])
{
  ...
  string master_server_for= "", string master_server_bind = "";
  ...
  inet_pton(AF_INET, (server_forward_info[0]).c_str(), &(master_dest.sin_addr));
  ...
  while(keep_running){
    ...
    while(!finished_probe_master){
      ...
      if(!get_prim){
        ...
        sendto(sock,  msg.c_str(), strlen(msg.c_str()), 0, (struct sockaddr*)&master_dest, sizeof(master_dest)); 
        get_prim = true;
      }
          ...
          while(true){
            ...
            if(start_with(line.c_str(), "GET_PRIM")){
              ...
              if(prim == self_id){
                ...
                sendto(sock,  req.c_str(), strlen(req.c_str()), 0, (struct sockaddr*)&master_dest, sizeof(master_dest)); 
              }else{
    ...
    if(primary_flag){
      ...
      prim_full_ip = (server_forward_info[prim_id])->ip + ":" + to_string((server_forward_info[prim_id])->port);
