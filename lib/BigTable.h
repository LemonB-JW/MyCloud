...
{
public:
	unordered_map<string, unordered_map<string, Cell*>> table; // row key: user name, col key: fileID (every folder has a unique ID: workerIndex + sequenceNum + timestamp
	// unordered_map<string, FileData*> files_in_path;
	BigTable(){

	};
	~BigTable(){
		for(unordered_map<string, unordered_map<string, FileData*>*>::iterator itr = table.begin();itr !=  table.end();itr++){
			delete itr->second;
		}
	}
	...
	bool delete(string row, string col);
