...
int main(int argc, char** argv) {
  ...
  // (use of InsecureChannelCredentials()).
	...
	std::string receiver = "shiqingliu1108@gmail.com";
	std::string created_time = "12/07/19";
	std::string subject = "Hello";
	int size = 1;
	std::string sender = "Jill";
	std::string content = "How are you?";

	std::string res = mailClient.sendMail(receiver, created_time, subject, size, sender, content);
	std::cout << "Reply from mail GRPC is " << res << std::endl;
